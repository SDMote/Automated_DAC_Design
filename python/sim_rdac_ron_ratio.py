...
import subprocess
import pdk
from rdac import rdac_tb, rdac, estimate_rdac_nl
...
for i in range(N):
    resolution[i] = int(RESOLUTION + 2*i)
    ...
    rdac(resolution[i], NMOS_W, PMOS_W, NG, RES_L*2**(2*i), 1)
    ...
    for j in range(M):
        ...
        inl2, dnl2, _, _ = estimate_rdac_nl(resolution[i], R, Rn_mean, Rp_mean)
    ...
    axs[i].plot(r_ratio, abs_inl2, 'r-o', label='estimated inl')
    axs[i].plot(r_ratio, abs_dnl2, 'r--', label='estimated dnl')
